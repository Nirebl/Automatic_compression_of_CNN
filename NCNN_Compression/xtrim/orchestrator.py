...
class XTrimOrchestrator:
    ...
    def run(self, max_candidates: Optional[int] = None) -> List[HistoryItem]:
        ...
        while True:
            ...
            try:
                ...
                print(f"Pareto size: {len(front)} / {len(history)}")
    ...
    def _bench_latency(self, ncnn_param: Path, ncnn_bin: Path, run_dir: Path) -> Dict[str, float]:
        backend = str(self.latency_cfg.backend).lower().strip()
    ...
    def _process_candidate(self, cand: CandidateConfig, run_dir: Path) -> HistoryItem:
        ...
        self._maybe_quantize_and_eval_int8(onnx_path, run_dir, extra, tag="before_qat")
        ...
        if do_qat:
            ...
            self._maybe_quantize_and_eval_int8(onnx_qat, run_dir, extra, tag="after_qat")
