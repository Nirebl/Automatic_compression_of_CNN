...
class AndroidTfliteBench:
    ...
    def __init__(self, tools: ToolsConfig, cfg: OrtAndroidBenchConfig, *, delegate: str = "xnnpack"):
        self.tools = tools
        self.cfg = cfg
        self.delegate = str(delegate or "xnnpack").strip().lower()
    ...
    def push_model(self, device: DeviceConfig, local_tflite: Path) -> str:
        rdir = self.cfg.remote_dir
        suffix = local_tflite.suffix or ".tflite"
        rmodel = f"{rdir}/model{suffix}"
        self.adb(device.serial, "push", str(local_tflite), rmodel)
        try:
    ...
    def run_once(
        ...
        *,
        ...
        cfg = self.cfg
        if not cfg.enabled:
            raise RuntimeError("TFLite Android benchmark is disabled because OrtAndroidBenchConfig.enabled=False")
        ...
        rmodel = self.push_model(device, local_tflite)
        ...
        while True:
            ...
            if data is not None:
                if "run_id" in data and str(data["run_id"]) != run_id:
                    ...
                    continue
                data.setdefault("backend", "tflite")
                data.setdefault("delegate", self.delegate)
                data.setdefault("runtime", f"tflite_{self.delegate}")
